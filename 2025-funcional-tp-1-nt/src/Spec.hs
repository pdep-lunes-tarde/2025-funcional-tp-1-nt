  ...
  describe "Parte 2: Temperaturas" $ do
    ...
    describe "fahrenheitACelsius" $ do
      it "pasa una temperatura de fahrenheit a celsius" $ do
        fahrenheitACelsius  33.8 `shouldBeEqualUpTo2Decimals` 1
    ...
    describe "fahrenheitACelsius y celsiusAFahrenheit son inversas" $ do
      ...
      it "convertir un valor en fahrenheit a celsius y luego volver a convertir a fahrenheit retorna el valor original" $ do
        celsiusAFahrenheit (fahrenheitACelsius 57) `shouldBe` 57
    ...
    describe "haceFrioCelsius" $ do
      ...
      it "Es verdad cuando hacen menos de 8 grados celsius" $ do
        shouldBe (haceFrioCelsius 7) True
      it "Hacen 8 grados celsius" $ do
        shouldBe (haceFrioCelsius 8) False
      it "Hacen mas de 8 grados celsius" $ do
        shouldBe (haceFrioCelsius 10) False
        
      it "Temperatura normal durante el invierno en el sur del pais" $ do
        pendingWith "Temperatura igual o por debajo de los 8 grados Celsius"
        haceFrioCelsius 8 `shouldBe` True
      it "Temperatura normal durante el invierno en el sur del pais" $ do
        pendingWith "Temperatura por encima de los 8 grados Celsius"
        haceFrioCelsius 15 `shouldBe` False

    describe "haceFrioFahrenheit" $ do
      -- Pista: hay 3 casos a testear
      it "Hacen menos de 46.4 grados fahrenheit" $ do
        shouldBe (haceFrioFahrenheit 40.4) True
      it "Hacen 46.4 grados fahrenheit" $ do
        shouldBe (haceFrioFahrenheit 46.4) True
      it "Hacen mas de 46.4 grados fahrenheit" $ do
        shouldBe (haceFrioFahrenheit 48.4) False
